...
def search(query: str, match_count: int = 5) -> dict:
    try:
        ...
        prompt = (
            ...
            f"Query: {query}\n\n"
            f"Excerpts:\n{context}\n\n"
            f"Answer in at least 4 sentences. If the excerpts don't contain enough information, say so clearly."
        )
        ...
        try:
            ...
            logger.warning(f"Gemini timeout for query: {query}")
            answer = "The response took too long to generate. Please try again."
